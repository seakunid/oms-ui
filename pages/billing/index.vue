          ...
          <form>
            <div style="display: flex">
              <div style="width: 50%; padding: 30px">
                <div class="form-group">
                  ...
                  <input
                    ...
                    @keydown="onChangeFullame"
                    v-model="fullname"
                  />
                  <p class="error-msg" v-if="errorMsg.fullname">
                    {{ errorMsg.fullname }}
                  </p>
                </div>
                <div class="form-group">
              ...
              <div style="width: 50%; padding: 30px">
                <div class="form-group">
                  <label for="price"
                    >Masukkan Biaya yang harus dibayar User</label
                  >
                  <input
                    type="text"
                    id="price"
                    ...
                    @keydown="onChangePrice"
                    v-model="price"
                  />
                  <p class="error-msg" v-if="errorMsg.price">
                    {{ errorMsg.price }}
                  </p>
                </div>
                <div class="form-group">
                  <label for="lastActiveDate"
                    >Tanggal Terakhir Aktif Layanan</label
                  >
                  <input
                    type="text"
                    id="lastActiveDate"
                    ...
                    @keydown="onChangelastActiveDate"
                    v-model="lastActiveDate"
                  />
                  <p class="error-msg" v-if="errorMsg.lastActiveDate">
                    {{ errorMsg.lastActiveDate }}
                  </p>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    validateInput() {
      !this.fullname
        ? (this.errorMsg.fullname = "Nama Lengkap User harus diisi")
      ...
      !this.price
        ? (this.errorMsg.price = `Biaya harus diisi`)
        : (this.errorMsg.price = "");
      !this.lastActiveDate
        ? (this.errorMsg.lastActiveDate =
            "Tanggal Terakhir Aktif layanan harus diisi")
        : (this.errorMsg.lastActiveDate = "");
    },
    clickSubmit() {
      this.validateInput();
    ...
    postRegisteredUser() {
      ...
      axios
        .post("https://seakun-mail-api-v1.herokuapp.com/billing", payload)
        .then(res => {
          if (res) {
