          ...
          <form>
            <div style="display: flex">
              <div style="width: 50%; padding: 30px">
                <div class="form-group">
                  ...
                  <input
                    ...
                    @keydown="onChangeName"
                    v-model="name"
                  />
                  <p class="error-msg" v-if="errorMsg.name">
                    {{ errorMsg.name }}
                  </p>
                </div>
                <div class="form-group">
              ...
              <div style="width: 50%; padding: 30px">
                <div class="form-group">
                  <label for="username"
                    >Username {{ $route.query.provider }}</label
                  >
                  <input
                    type="text"
                    id="username"
                    name="username"
                    class="form-control"
                    placeholder="Contoh: seakun.usermail@gmail.com"
                    @keydown="onChangeUsername"
                    v-model="username"
                  />
                  <p class="error-msg" v-if="errorMsg.username">
                    {{ errorMsg.username }}
                  </p>
                </div>
                <div class="form-group">
                  ...
                  <input
                    ...
                    name="password"
                    class="form-control"
                    placeholder="Contoh: kuY#123ah"
                    @keydown="onChangeUsername"
                    v-model="password"
                  />
                  <p class="error-msg" v-if="errorMsg.password">
                    {{ errorMsg.password }}
                  </p>
                </div>
                <div class="form-group">
                  ...
                  <input
                    ...
                    v-model="pin"
                  />
                  <p class="error-msg" v-if="errorMsg.pin">
                    {{ errorMsg.pin }}
                  </p>
                </div>
                <p
                  class="button-referral-code"
                  @click="useReferralCode = !useReferralCode"
                >
                  Use Referral Code
                </p>
            ...
            <button
              ...
              style="min-width: 9rem; margin-left: 30px"
            >
              <span
                v-if="isDisableBtn"
...
export default {
  ...
  methods: {
    ...
    validateInput() {
      !this.name
        ? (this.errorMsg.name = "Nama Lengkap User harus diisi")
        : (this.errorMsg.name = "");
      !this.email
        ? (this.errorMsg.email = "Email User harus diisi")
        : (this.errorMsg.email = "");
      this.packet == "Contoh: Paket Premium Grup"
        ? (this.errorMsg.packet = "Paket harus dipilih")
        : (this.errorMsg.packet = "");
      !this.username
        ? (this.errorMsg.username = `Username ${this.provider} harus diisi`)
        : (this.errorMsg.username = "");
      !this.password
        ? (this.errorMsg.password = `Password ${this.provider} harus diisi`)
        : (this.errorMsg.password = "");
      !this.pin
        ? (this.errorMsg.pin = `Pin ${this.provider} harus diisi`)
        : (this.errorMsg.pin = "");
    },
    clickSubmit() {
      this.validateInput();
    ...
    postRegisteredUser() {
      ...
      axios
        .post(
          "https://seakun-mail-api-v1.herokuapp.com/created-account",
          payload
        )
        .then(res => {
          if (res) {
